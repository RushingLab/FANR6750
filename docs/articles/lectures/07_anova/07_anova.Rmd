...
```{r echo = FALSE}
data(pinedata)
pine_wide <- tidyr::pivot_wider(pine, names_from = Treatment, values_from = Height)
...
pinedata$x <- c(0.8, 1, 1.2, 1.8, 2, 2.2, 2.8, 3, 3.2, 3.8, 4, 4.2)
ggplot(pinedata, aes(x = x, y = Height)) + 
...
```{r echo = TRUE}
fit1 <- lm(Height ~ Treatment, data = pinedata)
...
ggplot(pinedata, aes(x = x, y = Height)) + 
  ...
  scale_y_continuous("Height") +
  guides(color = FALSE) +
  geom_hline(yintercept = mean(pinedata$Height)) +
  annotate("text", label = "bar(y)[.]", x = 4.25, y = mean(pinedata$Height) + 0.6, 
           parse = TRUE, size = 8)
```
]
...
.pull-right[
```{r pinedata_gm, fig.width=4, fig.height=4}
library(dplyr)
ybari <- pinedata %>%
  group_by(Treatment) %>%
...
.pull-right[
```{r pinedata_var, fig.width=4, fig.height=4}
pinedata$ybar <- rep(ybari$y, each = 3)
ggplot(pinedata, aes(x = x, y = Height)) + 
  geom_point(size = 5) +
  scale_x_continuous("Fertilizer treatment", labels = c("A", "B", "C", "D"), breaks = 1:4) +
  scale_y_continuous("Height") +
  guides(color = FALSE) +
  geom_hline(yintercept = mean(pinedata$Height), color = "#446E9B") +
  geom_segment(data = ybari, aes(x = x - 0.25, xend = x + 0.25, y = y, yend = y)) +
  geom_segment(data = ybari, aes(x = x, xend = x, y = mean(pinedata$Height), yend = y), color = "#D47500") +
  geom_segment(data = pinedata, aes(x = x, xend = x, y = ybar, yend = Height), color = "#3CB521")
```
]
...
```{r pinedata_ssa, fig.width=8, fig.height=6}
ggplot(pinedata, aes(x = x, y = Height)) + 
  geom_point(aes(color = Treatment), size = 5) +
  scale_x_continuous("Fertilizer treatment", labels = c("A", "B", "C", "D"), breaks = 1:4) +
  scale_y_continuous("Height") +
  ...
  geom_hline(yintercept = mean(pinedata$Height)) +
  geom_segment(data = ybari, aes(x = x - 0.25, xend = x + 0.25, y = y, yend = y, color = Treatment)) +
  geom_segment(data = ybari, aes(x = x, xend = x, y= mean(pinedata$Height), yend = y))
```
...
```{r pinedata_ssw, fig.width=8, fig.height=6}
pinedata$ybari <- rep(ybari$y, each = 3)
ggplot(pinedata, aes(x = x, y = Height)) + 
  geom_point(aes(color = Treatment), size = 5) +
  scale_x_continuous("Fertilizer treatment", labels = c("A", "B", "C", "D"), breaks = 1:4) +
  scale_y_continuous("Height") +
...
```{r pinedata_sst, fig.width=8, fig.height=6}
ggplot(pinedata, aes(x = x, y = Height)) + 
  geom_point(aes(color = Treatment), size = 5) +
  scale_x_continuous("Fertilizer treatment", labels = c("A", "B", "C", "D"), breaks = 1:4) +
  scale_y_continuous("Height") +
  ...
  geom_segment(data = ybari, aes(x = x - 0.25, xend = x + 0.25, y = y, yend = y, color = Treatment)) +
  geom_segment(data = pinedata, 
               aes(x = x, xend = x, y = mean(pinedata$Height), yend = Height))
```
