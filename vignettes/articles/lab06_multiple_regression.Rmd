...
vignette: >
  %\VignetteIndexEntry{lab06_multiple_regression}
...
3) We would like to perform linear regression but we have more than one continuous predictor.
...
Some time ago, plantings of *Pinus caribaea* were established at four locations on Puerto Rico. As part of a study, four spacing intervals were used at each location to determine the effect of spacing on tree height. Twenty years after the plantings were established, measurements (height in ft) were made in the study plots. Additionally, soil chemistry was analyzed to determine average pH and nitrogen content (mg N/kg soil) for each tree. Note that although spacing interval is a number, it must be treated as a factor since these 4 spacing intervals (and only these 4) are of interest to the researcher.
...
1) Construct a model using the lm() function which predicts tree height as a function of spacing and, because the researcher is also interested in seeing if soil pH is contributing to differences in tree height, include pH as a predictor as well. You may choose to center the continuous variable or not, but remember that it will affect the way you interpret the results.
    ...
    d) Determine which tree spacings differ from each other with regard to their effect on tree height.
...
2) Construct an ANOVA table that shows the relationship between tree height and spacing as well as the other categorical variable of location. The researcher is not particularly interested in geographic location and she's not sure of the exact mechanism, but she thinks it may affect the results.
