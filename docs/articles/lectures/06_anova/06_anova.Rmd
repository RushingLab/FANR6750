---
title: "LECTURE 6: linear models part 2: categorical predictor > 2 levels"
...
pine$x <- c(0.8, 1, 1.2, 1.8, 2, 2.2, 2.8, 3, 3.2, 3.8, 4, 4.2)
ggplot(pine, aes(x = x, y = Height)) + 
...
ggplot(pine, aes(x = x, y = Height)) + 
  ...
  scale_y_continuous("Height") +
  guides(color = FALSE) +
  geom_hline(yintercept = mean(pine$Height)) +
  annotate("text", label = "bar(y)[.]", x = 4.25, y = mean(pine$Height) + 0.6, 
           parse = TRUE, size = 8)
```
]
...
.pull-right[
```{r pine_gm, fig.width=4, fig.height=4}
library(dplyr)
ybari <- pine %>%
  group_by(Treatment) %>%
...
.pull-right[
```{r pine_var, fig.width=4, fig.height=4}
pine$ybar <- rep(ybari$y, each = 3)
ggplot(pine, aes(x = x, y = Height)) + 
  geom_point(size = 5) +
  scale_x_continuous("Fertilizer treatment", labels = c("A", "B", "C", "D"), breaks = 1:4) +
  scale_y_continuous("Height") +
  guides(color = FALSE) +
  geom_hline(yintercept = mean(pine$Height), color = "#446E9B") +
  geom_segment(data = ybari, aes(x = x - 0.25, xend = x + 0.25, y = y, yend = y)) +
  geom_segment(data = ybari, aes(x = x, xend = x, y = mean(pine$Height), yend = y), color = "#D47500") +
  geom_segment(data = pine, aes(x = x, xend = x, y = ybar, yend = Height), color = "#3CB521")
```
]
...
ggplot(pine, aes(x = x, y = Height)) + 
  ...
  geom_hline(yintercept = mean(pine$Height)) +
  geom_segment(data = ybari, aes(x = x - 0.25, xend = x + 0.25, y = y, yend = y, color = Treatment)) +
  geom_segment(data = ybari, aes(x = x, xend = x, y= mean(pine$Height), yend = y))
```
...
ggplot(pine, aes(x = x, y = Height)) + 
  ...
  geom_segment(data = ybari, aes(x = x - 0.25, xend = x + 0.25, y = y, yend = y, color = Treatment)) +
  geom_segment(data = pine, 
               aes(x = x, xend = x, y = mean(pine$Height), yend = Height))
```
