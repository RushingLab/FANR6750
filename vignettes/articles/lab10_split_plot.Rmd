...
1) Conduct the appropriate ANOVA using both the `aov()` and `lme()` functions.

2) Give the appropriate additive model (this will be slightly different than the one for the split-plot design, since there is no blocking) and define each term. What are the associated null and alternative
hypotheses?
