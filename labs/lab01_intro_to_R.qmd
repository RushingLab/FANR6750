...
- John Fieberg's online [Statistics for Ecologists](https://statistics4ecologists-v3.netlify.app/) book

- [Data Analysis and Visualization in R for Ecologists](https://datacarpentry.github.io/R-ecology-lesson/introduction-r-rstudio.html)
...
- [Asking for help in R](https://rbook.flavioazevedo.com/asking-for-help)

- [Seeking help](https://datacarpentry.github.io/R-ecology-lesson/aio.html#aside-getting-help) from *Data Analysis and Visualization in R for Ecologists* 

# Using `R`- the very basics 

## The RStudio interface and panes

Although users can work directly in `R`, most choose to use RStudio which is the IDE (Integrated Development Environment) for `R`^[Note that you are still using `R` to do statistical analyses, visualizations, etc! So when writing a methods section, be sure to cite `R` (and any packages you use) rather than RStudio]. To use RStudio, you must first have `R` installed.
...
To see how data frames work, let's load a data frame called `jayData` that comes with the `FANR6750data` package. 
...
```{r eval = FALSE}
install_github("RushingLab/FANR6750data")
```
